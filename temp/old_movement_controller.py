...
class MovementController:
    def __init__(self):
        ...
        self._goal_s_bend_x = 0.0
        self._goal_s_bend_y = 0.0


        self._goal_curve_90_x = 0.0
        self._goal_curve_90_y = 0.0
        ...
        rospy.Subscriber('/vader/movement_controller/goal_s_bend', Float64MultiArray, self.goal_s_bend_callback)
        rospy.Subscriber('/vader/movement_controller/goal_stop', UInt8, self.goal_stop_callback)
        rospy.Subscriber('/vader/movement_controller/goal_curve_90', Float64MultiArray, self.goal_curve_90_callback)
    ...
    def goal_curve_90_callback(self, msg):
        if len(msg.data) != 2:
            rospy.logerr("Received goal_curve_90 message with incorrect data length: %s", len(msg.data))
            return
        if msg.data[0] == 0.0 or msg.data[1] == 0.0:
            rospy.loginfo("Received goal_curve_90 with zero values, ignoring.")
            return
        if msg.data[1] < 0.0:
            rospy.logerr("Received goal_curve_90 with negative y value: %s", msg.data[1])
            return

        # +ve x-value has the robot moving to the right, -ve x-value has the robot moving to the left
        self._goal_curve_90_x = msg.data[0]
        # y-value must be positive in this case, and represents forward movement
        self._goal_curve_90_y = msg.data[1]

    def goal_stop_callback(self, msg):
        if msg.data == 1:
            rospy.loginfo("Received goal_stop message, stopping the robot.")
            self.reset()
    ...
    def goal_s_bend_callback(self, msg):
        if len(msg.data) != 2:
            rospy.logerr("Received goal_s_bend message with incorrect data length: %s", len(msg.data))
            return
        if msg.data[0] == 0.0 or msg.data[1] == 0.0:
            rospy.loginfo("Received goal_s_bend with zero values, ignoring.")
            return
        if msg.data[1] < 0.0:
            rospy.logerr("Received goal_s_bend with negative y value: %s", msg.data[1])
            return

        # +ve x-value has the robot moving to the right, -ve x-value has the robot moving to the left
        self._goal_s_bend_x = msg.data[0]
        # y-value must be positive in this case, and represents forward movement
        self._goal_s_bend_y = msg.data[1]
